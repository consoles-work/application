...
## Этап 8: Локализация — TODO

### 8.1 Выбор библиотеки
...
- Контекстные меню, тулбары, хинты

### 8.3 Что НЕ переводится
- Данные пользователя (имена воркспейсов, консолей, wiki-контент)
- Технические идентификаторы (ssh, local, bash, zsh)

### 8.4 Переключение языка
- Settings → "Интерфейс" → кнопки ru / en (заготовка уже есть)
